...
# Prefer reading endpoint from environment so it's configurable in different environments
phoenix_endpoint = os.environ.get("PHOENIX_COLLECTOR_ENDPOINT", "https://app.phoenix.arize.com/v1/traces")
# strip possible surrounding quotes from .env values
phoenix_endpoint = phoenix_endpoint.strip().strip("'\"")
...
def inspector_node(state: State) -> Literal["breakfast_chef", "lunch_chef", "dinner_chef", "__end__"]:
    ...
    # Inspector logic is instrumented via OTEL; no manual Arize logging here.

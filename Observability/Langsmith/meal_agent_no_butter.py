...
@traceable
def format_prompt(system_text: str, messages: list[BaseMessage]):
    """Build a prompt combining a system message and existing messages."""
    return [SystemMessage(content=system_text)] + messages


@traceable(run_type="llm")
def invoke_llm(llm, messages: list[BaseMessage]):
    """Invoke an LLM safely through a traceable wrapper.

    Returns whatever the underlying LLM returns (often an AIMessage-like object).
    """
    if llm is None:
...
def parse_output(response):
    """Extract a plain string from an LLM response object."""
    # Many LLMs used here return objects with a `.content` attribute.
    try:
...
def router_node(state: State) -> Literal["breakfast_chef", "lunch_chef", "dinner_chef", "general_chat"]:
    ...
    response = invoke_llm(llm_nano, prompt)
...
def breakfast_chef_node(state: State):
    ...
    response = invoke_llm(llm_mini, prompt)
    parsed = parse_output(response)
    return {"messages": [AIMessage(content=f"**Breakfast Chef:**\n{parsed}")], "active_chef": "breakfast_chef"}
...
def dinner_chef_node(state: State):
    ...
    response = invoke_llm(llm_dinner, prompt)
    parsed = parse_output(response)
    return {"messages": [AIMessage(content=f"**Dinner Chef:**\n{parsed}")], "active_chef": "dinner_chef"}

def general_chat_node(state: State):
    if llm_nano is None: return {"messages": []}
    messages = state["messages"]
    response = invoke_llm(llm_nano, messages)
    parsed = parse_output(response)
    return {"messages": [AIMessage(content=parsed)], "active_chef": "general_chat"}
